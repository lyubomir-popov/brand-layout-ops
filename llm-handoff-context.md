...
Lane I is now active: replace the remaining `sceneFamilyConfigs` persistence bridge with graph-first per-family document persistence while keeping backward-compatible load and automation apply paths.
...
Continue work in `c:\Users\lyubo\work\repos\brand-layout-ops` using `c:\Users\lyubo\work\repos\racoon-anim` as the reference app. Read `AGENTS.md`, `llm-handoff-context.md`, `docs/TODO.md`, `docs/product-roadmap.md`, and `README.md` first. Treat this handoff as the cold-start summary and `docs/TODO.md` as the canonical architecture backlog. Stage 1 parity, the selected-operator pane lane, the baseline-foundry shell-compliance lane, and the preset-residue cleanup are now complete. The active queue is Lane I: move document persistence from the current `backgroundGraph` plus `sceneFamilyConfigs` compromise toward graph-first per-family persistence while keeping backward-compatible load/apply behavior. Keep `baseline-foundry` read-only unless shared shell or style work clearly belongs upstream.