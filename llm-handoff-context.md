...
- CSV authoring now exists as an import and staging path, and the next document-specific follow-up is EQ-12: expose the saved background chain in a list-first network view instead of pushing deeper into CSV-first editing or ad hoc preview glue
- The graph/runtime layer now has a document-owned active background chain through `project.backgroundGraph`, and the missing next seam is inspector UI: the preview still needs a list-first network surface for selecting nodes and exposing their typed parameter panes before any fuller graph editor
...
Items EQ-1 through EQ-12 are the user-directed priorities in dependency order:
...
12. **EQ-12** — Add the next-stage list-first network view now that EQ-10 and EQ-11 are stable; graph UI still comes after persisted operator chaining, not before.

### Immediate next steps

**EQ-12 follow-up** — expose the saved background chain in a list-first network view so the inspector can select nodes and show only the active operator's typed controls before attempting any fuller graph UI.
...
- [x] Control-panel toggle cleanup: removed the external `Controls` button, kept the panel on a single visibility model across breakpoints, and wired `Tab` to hide or show it everywhere
...
- [ ] Persistent local document model and file-backed projects
  ...
  - Landed follow-up: shared `document.project.sceneFamilyConfigs` now persists those non-halo scene-family settings through save/save-as/duplicate/reopen plus source-default reset or writeback, while source-default authoring writes the shared overlay-document payload and still reads older raw snapshots
