...
- Recent-document reopen is now wired through stored file handles, preview documents now persist the shared overlay document metadata/state envelope plus shared project metadata for scene-family selection and document targets, preview-local document workspace orchestration now lives in `apps/overlay-preview/src/document-workspace.ts`, preview-side snapshot or apply plumbing now lives in `apps/overlay-preview/src/preview-document-bridge.ts`, and the Output Format panel now edits scene family plus add or delete document sizes against that shared schema, so the next document-specific follow-up is making non-halo scene families drive renderer or parameter-surface behavior instead of stopping at persisted metadata and target management.
...
- CSV authoring now exists as an import and staging path, and the next document-specific follow-up is making the selected scene family affect renderer or parameter behavior rather than pushing deeper into CSV-first editing
...
- [ ] Persistent local document model and file-backed projects
  ...
  - Next: use the chosen document scene family to drive renderer and parameter-surface behavior instead of only storing it as document metadata
