...
- [ ] Persistent local document model and file-backed projects
  ...
  - Landed follow-up: the selected document scene family now changes the stage renderer and composed-frame export path, and `apps/overlay-preview/src/scene-family-preview.ts` now gives phyllotaxis and fuzzy-boids richer family-specific rendering instead of only flat adapter dots
  - Landed follow-up: dedicated operator-owned parameter surfaces now exist for phyllotaxis and fuzzy-boids instead of only a non-halo preview summary
