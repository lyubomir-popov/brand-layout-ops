...
- The stage renderer and composed-frame export path now respect the selected document scene family: `halo` keeps the Three.js halo renderer, while `phyllotaxis` and `fuzzy-boids` draw point-field previews through `apps/overlay-preview/src/scene-family-preview.ts`, and the inspector swaps the halo controls for a scene-family preview summary when halo is not selected
...
- CSV authoring now exists as an import and staging path, and the next document-specific follow-up is giving phyllotaxis or fuzzy-boids real per-family controls or a fuller render path rather than pushing deeper into CSV-first editing
...
- [ ] Persistent local document model and file-backed projects
  ...
  - Landed follow-up: the selected document scene family now changes the stage renderer and composed-frame export path, and non-halo families replace the halo inspector with a preview summary instead of irrelevant halo controls
  - Next: add dedicated parameter surfaces or a richer preview renderer for non-halo scene families instead of leaving them on adapter defaults
