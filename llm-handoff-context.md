...
- Lane P is active: P1-P3 are landed and P4 has started. P4b now defines the built-in preset contract as one seed package for frame, safe area, and grid, while document buckets stay authoritative for later overrides. The next pass should keep extending that split between global size presets, document-owned format variants, and future output recipes while preserving the existing saved-file shape until the larger schema redesign is settled.
