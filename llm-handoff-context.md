...
- **Lane A — GPU/export seam validation:** validate the experimental GPU fuzzy-boids path (`?gpuBoids=1`) and decide whether to productize it or keep it as research; current guardrails now avoid animated-seed reset loops and route unsupported top-k neighbor configs back to CPU.
