...
- **Shell contract discipline** — do not add new local shell class contracts or new `[data-*]` style selectors in overlay-preview. Lane F is now the active cleanup pass for removing the remaining ones.
...
Continue work in `c:\Users\lyubo\work\repos\brand-layout-ops` using `c:\Users\lyubo\work\repos\racoon-anim` as the reference app. Read `AGENTS.md`, `llm-handoff-context.md`, `docs/TODO.md`, `docs/product-roadmap.md`, and `README.md` first. Treat this handoff as the cold-start summary and `docs/TODO.md` as the canonical architecture backlog. Focus on the active queue in the TODO: Lane F shell compliance, now that the selected-operator pane work is complete. Treat Stage 1 parity and halo as closed unless a concrete regression appears. Keep documents as the real working unit, keep `backgroundGraph` authoritative for background execution, and keep `baseline-foundry` read-only unless shared shell or style work clearly belongs upstream.