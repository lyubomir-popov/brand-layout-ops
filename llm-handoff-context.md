...
- Recent-document reopen is now wired through stored file handles, preview documents now persist the shared overlay document metadata/state envelope plus shared project metadata for scene-family selection and document targets, preview-local document workspace orchestration now lives in `apps/overlay-preview/src/document-workspace.ts`, and preview-side snapshot or apply plumbing now lives in `apps/overlay-preview/src/preview-document-bridge.ts`, so the next document-specific follow-up is consuming that shared project metadata in the UI instead of inventing another preview-local schema.
...
- CSV authoring now exists as an import and staging path, and the next document-specific follow-up is wiring the shared document project metadata into scene-family or multi-target editing flows rather than pushing deeper into CSV-first editing
...
- [ ] Persistent local document model and file-backed projects
  ...
  - Next: consume that shared document project metadata in the preview for scene-family selection or document target management instead of leaving it as passive persisted metadata
