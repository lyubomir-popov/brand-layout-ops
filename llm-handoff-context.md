...
- Recent-document reopen is now wired through stored file handles, preview documents now persist the shared overlay document metadata/state envelope plus shared project metadata for scene-family selection and document targets, preview-local document workspace orchestration now lives in `apps/overlay-preview/src/document-workspace.ts`, preview-side snapshot or apply plumbing now lives in `apps/overlay-preview/src/preview-document-bridge.ts`, the Output Format panel now edits scene family plus add or delete document sizes against that shared schema, and non-halo scene families now render through `apps/overlay-preview/src/scene-family-preview.ts`, so the next document-specific follow-up is moving those scene-family controls out of preview-local state and into document-owned or shared contracts.
...
- CSV authoring now exists as an import and staging path, and the next document-specific follow-up is moving phyllotaxis, fuzzy-boids, and scatter settings into document-owned state rather than pushing deeper into CSV-first editing
...
- Fuzzy boids inspector panel landed: `apps/overlay-preview/src/fuzzy-boids-section.ts` now exposes flock, speed, force, scale, mass, and bounds controls through preview state instead of hardcoded scene-family defaults (commit `25fed2c`)
- Phyllotaxis inspector panel landed: `apps/overlay-preview/src/phyllotaxis-section.ts` now exposes point count, radius, radius falloff, angle offset, and animation controls through preview state instead of hardcoded scene-family defaults
...
3. **EQ-3** — ~~Fuzzy boids parameter panel (numBoids, separation, alignment, cohesion, bounds, etc.).~~ **DONE** (commit `25fed2c`). Added operator-owned `fuzzy-boids` panel with live simulation controls backed by preview state.
...
**Anti-drift follow-up** — move phyllotaxis, fuzzy-boids, and scatter settings out of preview-local state and into document-owned or shared scene-family contracts.
...
- [ ] Persistent local document model and file-backed projects
  ...
  - Landed follow-up: the selected document scene family now changes the stage renderer and composed-frame export path, and `apps/overlay-preview/src/scene-family-preview.ts` now gives phyllotaxis, fuzzy-boids, and scatter richer family-specific rendering instead of only flat adapter dots
  - Landed follow-up: dedicated operator-owned parameter surfaces now exist for phyllotaxis, fuzzy-boids, and scatter instead of only a non-halo preview summary
  - Next: move those non-halo scene-family settings out of preview-local state and into document-owned or shared scene-family contracts
