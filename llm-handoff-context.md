...
- The rebuild has a usable overlay editor and shared package-backed control surface. Stage 1 parity is now complete: overlay text, logo placement, selected-element editing, document workflow, export pipeline, and the export-relevant geometry screenshot pass all match the current reference closely enough to treat parity as closed.
- A first-pass local document workflow now exists, so authored text, placement, logo state, and scene configuration can be saved and reopened as real local project files instead of living only in browser storage.
...
- The real working artifact is now a local `.brand-layout-ops.json` file rather than browser-local preset state. Documents persist shared overlay metadata plus shared `project` state such as scene family, targets, `sceneFamilyGraphs`, and the live active `backgroundGraph`; non-halo preview execution evaluates that active graph through preview-runtime operator definitions backed by the shared graph runtime. Legacy `sceneFamilyConfigs` survives only as a load/apply compatibility bridge for older documents and automation payloads. Chromium file save/open currently uses a `.json` picker filter for compatibility while suggested filenames still use `.brand-layout-ops.json`, and the later user retest showed the observed Chrome failure was profile-specific rather than a current repo regression.
...
No approved execution lane is currently open. The repo is caught up through the preset-residue cleanup and the graph-first family-persistence migration.
...
- `main.ts` is ~833 lines — authoring, export, source-default, CSV draft, playback, overlay editing, config-editor rebuild logic, document target CRUD, profile/content switching, preview-document build/apply/reset flows, background-graph selection/sync, stage rendering/composition, and shell bootstrap/workspace chrome are extracted, with halo config merge helpers moved to `operator-halo-field`; remaining concentration is DOM query helpers, overlay-visibility shell glue, and final composition/bootstrap wiring.
- `project.backgroundGraph` vs `sceneFamilyGraphs` — `sceneFamilyGraphs` now persists all families and `backgroundGraph` mirrors the active family for runtime convenience. Remaining cleanup is optional: if we want fully normalized files later, derive `backgroundGraph` at load time and omit it from the serialized envelope.
...
- **Background graph authority** — `sceneFamilyGraphs` is the persisted per-family store, and `backgroundGraph` is the live active-family projection used by the preview runtime. Legacy `sceneFamilyConfigs` exists only as a backward-compat load/apply bridge. Do not re-introduce per-edit mirroring.
...
| Overlay param cloning + export defaults | `apps/overlay-preview/src/sample-document.ts` |
...
- **Profiles, formats, defaults:** `racoon-anim/src/app/config-schema.js`, `default-config-source.js`, `editor-constants.js`, `index.js`
...
Continue work in `c:\Users\lyubo\work\repos\brand-layout-ops` using `c:\Users\lyubo\work\repos\racoon-anim` as the reference app. Read `AGENTS.md`, `llm-handoff-context.md`, `docs/TODO.md`, `docs/product-roadmap.md`, and `README.md` first. Treat this handoff as the cold-start summary and `docs/TODO.md` as the canonical architecture backlog. Stage 1 parity, the selected-operator pane lane, the baseline-foundry shell-compliance lane, the preset-residue cleanup, and the graph-first family-persistence migration are now complete. Unless the user promotes a new queue item, treat the repo as caught up and only pursue optional follow-ups such as content-format vs project variants or serialized-envelope cleanup that derives `backgroundGraph` from `sceneFamilyGraphs`. Keep `baseline-foundry` read-only unless shared shell or style work clearly belongs upstream.