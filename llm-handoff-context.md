...
- Recent-document reopen is now wired through stored file handles, preview documents now persist the shared overlay document metadata/state envelope with backward compatibility for earlier preview-local files, and preview-local document workspace orchestration now lives in `apps/overlay-preview/src/document-workspace.ts`, so the next document-specific follow-up is pushing the remaining preview-state serialization or application rules further down from `main.ts`.
...
- Preview document files now persist shared `operator-overlay-layout` document metadata/state envelopes while `apps/overlay-preview/src/preview-document.ts` keeps the preview-only preset and pending-CSV extras plus legacy-file compatibility for the older preview-local envelope
- Preview-local document workspace state, recent-document rendering, file open or save orchestration, and download fallback now live in `apps/overlay-preview/src/document-workspace.ts` instead of `apps/overlay-preview/src/main.ts`, so the remaining document-specific preview seam is mostly preview-state serialization or application plus preview-only extras
...
- CSV authoring now exists as an import and staging path, and the next document-specific follow-up is moving the remaining preview-side document serialization or application rules out of `apps/overlay-preview/src/main.ts` rather than pushing deeper into CSV-first editing
...
- [ ] Persistent local document model and file-backed projects
  ...
  - Next: continue moving the remaining preview-side document serialization or state-application rules out of `apps/overlay-preview/src/main.ts` now that the persisted core document shape and workspace controller are split
  - Next: shape the broader document model so one document can own a swappable scene-family or background operator stack such as halo, boids, or phyllotaxis, plus one or more target output sizes and export settings
...
Continue work in `c:\Users\lyubo\work\repos\brand-layout-ops` using `c:\Users\lyubo\work\repos\racoon-anim` as the reference app. Read `AGENTS.md`, `llm-handoff-context.md`, `docs/rebuild-plan.md`, `docs/product-roadmap.md`, and `README.md` first. The current product direction is explicit: documents, not browser presets, are the real unit of work. A document should eventually own the swappable scene-family or background operator stack such as halo, boids, or phyllotaxis, plus one or more target output sizes, layout state, and export settings. The preview already opens, saves, save-as, duplicates, and reopens recent `.brand-layout-ops.json` files through `apps/overlay-preview/src/preview-document.ts`, which stores the shared overlay document metadata/state envelope and keeps backward compatibility for earlier preview-local files while still carrying preview-only extras. Preview-local file orchestration, recent-document handling, dirty-state UI, and fallback download behavior now live in `apps/overlay-preview/src/document-workspace.ts` instead of `apps/overlay-preview/src/main.ts`. Startup no longer seeds the working state from browser-local preset storage, but localStorage mirroring still exists inside the preview shell and should not become the canonical model again. CSV authoring is now a secondary import and staging path, not the main editing seam. The single-frame PNG halo export regression is manually revalidated and no longer the active blocker. Continue moving the remaining preview-side document serialization or state-application rules out of `apps/overlay-preview/src/main.ts`, and sketch the next shared document shape for scene-family selection plus multiple output targets. Keep parity-first and anti-drift discipline, keep the preview as an adapter, keep `portable-vertical-rhythm` as the styling owner, and if you touch stacked sliders or shell primitives, coordinate with `c:\Users\lyubo\work\repos\portable-vertical-rhythm`, where the shared narrow-panel stacked-slider follow-up is already documented in `llm-handoff-context.md` and `todo.md`.
